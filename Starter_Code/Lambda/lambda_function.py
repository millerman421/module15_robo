...
def recommend_portfolio(intent_request):
  ...
  # Gets the invocation source, for Lex dialogs "DialogCodeHook" is expected.
        ...
        if investment_amount is not None:
            ...
            if investment_amount < 5000:
                return build_validation_result(
                    False,
                    "investment_amount",
                    "Please re-renter an amount that is greater than $5000",
            )

    # Validate the age is not less than 0 or greater than 65
        if age is not None:
            age = parse_int(
                age
            )  # Since parameters are strings it's important to cast values
    ...
    risk_level_lowercase = str(risk_level).lower()

    if risk_level_lowercase == "none":
        result = "100% bonds (AGG), 0% equities (SPY)"
    elif risk_level_lowercase == "low":
        result = "60% bonds (AGG), 40% equities (SPY)"
    elif risk_level_lowercase == "medium":
        result = "40% bonds (AGG), 60% equities (SPY)"
    else:
        result = "20% bonds (AGG), 80% equities (SPY)"
    ...
    return close(
        ...
        "Fulfilled",
        {
            ...
            "content": """Thank you for your information; based on a {} risk level your recommended portfolio allocation is as follows:{}.
            """.format(
                risk_level_lowercase, result
            ),
        },
    )
